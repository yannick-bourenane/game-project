...
function startGame() {
  ...
  class Movable extends MapElement {
    ...
    mooveTo(key) {
      ...
      if (!defeatStatus) this.updatePosition(xNew, yNew, direction);
    }
  ...
  class Enemy extends Movable {
    ...
    mooveEnemies() {
      ...
      loopMooves();
      //clearInterval(mooveIntervalId)
      mooveIntervalId = setInterval(function () {
        loopMooves();
      }, sequenceTotal);
    }
  }
  // Size of an element is currently 3*3
  const player1 = new Doggy("Rex", "dog", 1, 1, 5);
  const tree = {};
  const cat = {};
  const enemy = {};
  ...
  function checkEmpty(name, type, x, y) {
    ...
    if (updatedMap[x][y] === `` || updatedMap[x][y].includes(`${name}`)) {
      ...
      (updatedMap[x][y].includes(`enemy`) ||
        updatedMap[x][y].includes(`dog`)) && !updatedMap.includes(`dog`)
    ) {
      defeatStatus = true;
      ...
      defeatTimeOutId = setTimeout(function () {
        defeat();
      }, 1500);
      //defeat();
  ...
  targetMap.addEventListener("keydown", e => {
    ...
    if (keyMoove.includes(e.key) && !defeatStatus) {
